...
function Login({ onToggle }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    const res = await axios.post('http://localhost:3000/login', { email: formData.email, password: formData.password }, {
      headers: {
    ...
    try {
      if (res) {
        setTimeout(() => {
          ...
          const name = localStorage.setItem('name', JSON.stringify(res.data.name));
          navigate('/')
          console.log(res);
          setFormData({
            email: '',
  ...
  return (
    <div className="h-[80vh] bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="login-container bg-white/80 backdrop-blur-lg rounded-2xl shadow-2xl border border-white/20 overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="login-form">
          ...
          <button type="submit" className={`submit-btn bg-gradient-to-r from-blue-600 to-purple-600 text-white ${isLoading ? 'loading' : ''}`} disabled={isLoading}>
            {isLoading ? (
              <>
